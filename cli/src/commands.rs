...
pub async fn run(command: Command, api_base: Option<&str>, cluster: Option<&str>) -> Result<()> {
    match command {
        Command::Auth(a) => match a.cmd {
            ...
            AuthCommand::Status => match ApiClient::new(None, api_base, cluster) {
            ...
            match t.cmd {
                TunnelCommand::List => {
                    for t in client.list_tunnels().await? {
                        println!(
                            "{}\t{}\t{}\t{}\t{}",
                            t.tunnel_id, t.name, t.description, t.tunnel_expiration, t.port_count
                        );
                    }
                }
                TunnelCommand::Create {
                    ...
                    output::kv(&[
                        ...
                        ("Expiration hours", t.expiration_hours.to_string()),
                    ]);
                }
                TunnelCommand::Show { tunnel_id: id } => {
                    ...
                    output::kv(&[
                        ...
                        ("Name", t.name),
                        ("Description", t.description),
                        ("Expiration", t.tunnel_expiration.to_string()),
                    ]);
                }
                TunnelCommand::Update {
                    tunnel_id: id,
                    name,
                    description,
                    expiration,
                ...
                TunnelCommand::Delete { tunnel_id: id } => {
                    ...
                    if config::load()?.default_tunnel_id.as_deref() == Some(&id) {
                ...
                TunnelCommand::Set { tunnel_id } => {
                    client.show_tunnel(&tunnel_id).await?;
        ...
        Command::Port(p) => {
            ...
            match p.cmd {
                PortCommand::List { tunnel_id: id } => {
                    for p in client.list_ports(&tunnel_id(id)?).await? {
                        println!(
                            "{}\t{}\t{}\t{}",
                            p.port, p.protocol, p.allow_anonymous, p.tunnel_id
                        );
                    }
                }
                PortCommand::Create {
                    ...
                    client
                        .create_port(&tunnel_id(id)?, port, &protocol, allow_anonymous)
                        .await?;
                ...
                PortCommand::Show {
                    ...
                    output::kv(&[
                        ("Tunnel ID", p.tunnel_id),
                        ("Port", p.port.to_string()),
                        ("Protocol", p.protocol),
                        ("Allow anonymous", p.allow_anonymous.to_string()),
                    ]);
                }
                PortCommand::Update {
                    tunnel_id: id,
                    ...
                    if allow_anonymous && deny_anonymous {
                        bail!("--allow-anonymous and --deny-anonymous are mutually exclusive");
                    }
                    client
                        .update_port(&tunnel_id(id)?, port, allow_anonymous && !deny_anonymous)
                        .await?;
                ...
                PortCommand::Delete {
                    ...
                    client.delete_port(&tunnel_id(id)?, port).await?;
        ...
        Command::Limits => {
            ...
            output::kv(&[
                ("Quota", output::bytes(l.quota_bytes)),
                ("Used", output::bytes(used)),
                ("Active tunnels", l.active_tunnels.to_string()),
                ("Max tunnels", l.max_tunnels.to_string()),
                ("Max ports/tunnel", l.max_ports_per_tunnel.to_string()),
                ("Max hosts/tunnel", l.max_hosts_per_tunnel.to_string()),
            ]);
        }
...
async fn run_ping(args: PingArgs) -> Result<()> {
    ...
    loop {
        ...
        match client.get(&args.uri).send().await {
            ...
            Err(e) => {
                println!("HTTP ERROR -- {} ms ({e})", start.elapsed().as_millis());
                return Ok(());
            }
        }
        tokio::select! {
